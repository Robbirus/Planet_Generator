...
public class Planet : MonoBehaviour
{
    ...
    {
        Initialize();
        GenerateMesh(); // L'ancienne méthode synchrone d'origine
        GenerateColours();
    }

    // Generate a planet including shape and color settings
    // onFaceProgress(facesDone, totalFaces) is called after each face so
    // LevelLoader can animate the progress bar smoothly.
    public IEnumerator GeneratePlanetAsync(System.Action<int, int> onFaceProgress = null)
    {
        Initialize();
        yield return StartCoroutine(GenerateMeshAsync(onFaceProgress));
        GenerateColours();
    }
    ...
    public void GeneratePlanetRuntime(ShapeSettings shape, ColourSettings colour)
    {
        ...
        CleanupMeshChildren(); // remove old meshes if any
    ...
    // Generate mesh for each terrain face, yielding between each one so
    // Unity can update the loading bar between heavy operations.
    public IEnumerator GenerateMeshAsync(System.Action<int, int> onFaceProgress = null)
    {
        int activeFaces = 0;
        for (int i = 0; i < 6; i++)
            if (meshFilters[i].gameObject.activeSelf) activeFaces++;
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            onFaceProgress?.Invoke(done, activeFaces);
            yield return null;  // one yield per face is enough
        }
        colourGenerator.UpdateElevation(shapeGenerator.elevationMinMax);
    }

    // Generate mesh for each terrain face (editor)
    void GenerateMesh()
    {
        for (int i = 0; i < 6; i++)
        {
